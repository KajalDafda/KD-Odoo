...
class ResConfigSettings(models.TransientModel):
    ...
    ai_kdchatbot_provider = fields.Selection([
        ...
        config_parameter='ai_kdchatbot.provider',
        default='gemini',
    ...
    def _selection_ai_kdchatbot_model(self):
        ...
        provider = self.ai_kdchatbot_provider if self else False
    ...
    @api.onchange('ai_kdchatbot_provider')
    def _onchange_ai_kdchatbot_provider_reset_model(self):
        ...
        valid_values = {v for v, _ in MODEL_CHOICES.get(self.ai_kdchatbot_provider, [])}
        if self.ai_kdchatbot_model not in valid_values:
            self.ai_kdchatbot_model = MODEL_CHOICES.get(self.ai_kdchatbot_provider, [(False, '')])[0][0]

    ai_kdchatbot_api_key = fields.Char(
        string='AI API Key',
        config_parameter='ai_kdchatbot.api_key',
    ...
    ai_kdchatbot_api_url = fields.Char(
        string='AI API Endpoint',
        config_parameter='ai_kdchatbot.api_url',
    ...
    ai_kdchatbot_model = fields.Selection(
        selection='_selection_ai_kdchatbot_model',
        string='Model',
        config_parameter='ai_kdchatbot.model',
    ...
    ai_kdchatbot_model_custom = fields.Char(
        string='Custom Model Override (optional)',
        config_parameter='ai_kdchatbot.model_custom',
    ...
    ai_kdchatbot_allowed_models = fields.Char(
        string='Allowed Odoo Models (optional)',
        config_parameter='ai_kdchatbot.allowed_models',
