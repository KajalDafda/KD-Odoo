...
SESSION_KEY = 'ai_chatbot_mcp_oauth_states'
...
class AiChatMcpOAuthController(http.Controller):
    ...
    def _redirect_uri(self):
        ...
        return f'{base_url}/ai_chatbot/mcp/oauth/callback'

    @http.route('/ai_chatbot/mcp/oauth/connect', type='http', auth='user')
    ...
    @http.route('/ai_chatbot/mcp/oauth/callback', type='http', auth='user')
    def callback(self, code=None, state=None, error=None, **kwargs):
        ...
        return werkzeug.utils.redirect('/odoo/action-ai_chatbot.action_ai_chat_mcp_server')
